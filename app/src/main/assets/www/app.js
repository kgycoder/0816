...
function updFavBtn() {
    ...
    ['b-fav', 'np-fav'].forEach(id => {
        ...
        // Set the SVG path's fill/stroke via attribute (not .style) — this is the same
        // technique refreshDots() already uses for the card favorite icons, and it wins
        // over both the inherited "fill=none" presentation attribute on the parent <svg>
        // and any pre-existing !important CSS rule that only fires on '.on', so the heart
        // reliably colors in immediately regardless of which one would otherwise apply.
        // When off, the attributes are removed entirely so each icon falls back to its own
        // original default (np-fav and b-fav don't share the same default stroke color).
        btn.querySelectorAll('svg path').forEach(p => {
            if (f) { p.setAttribute('fill', 'var(--acc)'); p.setAttribute('stroke', 'var(--acc)'); }
